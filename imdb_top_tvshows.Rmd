...
imdb_table <- 
  ...
  tab_footnote(
    footnote = md(glue("The line shows the average IMDb rating per season as the series progresses. Each vertical stripe represents one episode colored by the difference to the series' average (normalized). The plots are scaled to a maximum of {unique(table_trends$ratings[[1]]$max_x)} episodes; out of the top 250 only “The Simpsons” crossed that mark.<br>Ranks are compared to a dataset of the IMDb Top 250 TV series from May 2019.")),
  ...
  cols_label(
    ...
    trends = html("<span style='line-height:.65;'>Rating Trends:<br><i style='color:grey;font-weight:400;'>Are episodes <b style='color:#ffa500;'>worse</b> or <b style='color:#02af74;'>better</b> than the series’ average? </i></span>")
  ) %>%
  ## Column widths
  cols_width(
    vars(rank) ~ px(20),
...
```{r save-table}
#gtsave(imdb_table, "outpu/MDb_Top250.html", inline_css = TRUE)
#gtsave(imdb_table, "outpu/IMDb_Top250.png", expand = 20)
```
